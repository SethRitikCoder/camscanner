...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _loadLocalFiles() async {
    try {
      ...
      if (_selectedFolderId != null && _folders.isNotEmpty) {
        final folderIndex = _folders.indexWhere((f) => f.id == _selectedFolderId);
        if (folderIndex != -1) {
          activeFolderName = _folders[folderIndex].name;
        }
      }

      final dir = await StorageService.getPublicDirectory(folderName: activeFolderName);
      List<File> filesList = [];

      if (_selectedFolderId == null) {
        // Recursively list all files under MyScannerApp (across all folders)
        ...
        // List only files under the selected folder directory
        if (await dir.exists()) {
      ...
      // Fetch all metrics asynchronously and build metadata cache
      ...
      for (var file in filesList) {
        try {
          ...
          final formattedDate = "${modifiedTime.year}-${modifiedTime.month.toString().padLeft(2, '0')}-${modifiedTime.day.toString().padLeft(2, '0')} ${modifiedTime.hour.toString().padLeft(2, '0')}:${modifiedTime.minute.toString().padLeft(2, '0')}";
  ...
  String _cleanDisplayTitle(String fileName) {
    // Strip extension (.pdf, .jpg, .jpeg, .png)
    String nameWithoutExt = fileName.replaceAll(RegExp(r'\.(pdf|jpg|jpeg|png)$', caseSensitive: false), '');

    // Look for a milliseconds timestamp (10 to 13 digits) in the file name to format it pretty
  ...
  Future<void> _startScan() async {
    try {
      ...
      if (result.images.isNotEmpty) {
        ...
        final title = "DocScanner ${DateFormat('MM-dd-yyyy HH:mm').format(DateTime.now())}";
      ...
      if (errorStr.contains('cancelled') || errorStr.contains('operation cancelled')) {
        return;
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
            ...
            onPressed: () async {
              ...
              if (name.isNotEmpty) {
                await DatabaseHelper.instance.insertFolder(
                  FolderModel(name: name, createdAt: DateTime.now().millisecondsSinceEpoch),
                );
                if (context.mounted) {
                    ...
                    child: Row(
                      children: [
                        ...
                        TextButton.icon(
                          ...
                          label: const Text("+ Folder", style: TextStyle(color: Color(0xFF00A86B))),
                        ),
                      ],
                    ),
                  ),
                    ...
                    child: _localFiles.isEmpty
                        ? Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.folder_open_outlined, size: 70, color: Colors.grey.shade400),
                                ...
                                child: ListTile(
                                  ...
                                  leading: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      borderRadius: BorderRadius.circular(8),
                                      color: isPdf
                                          ? const Color(0xFFFEE2E2) // Light red for PDF
                                          : const Color(0xFFECFDF5), // Light green for images
                                    ),
                                    child: ClipRRect(
                                      borderRadius: BorderRadius.circular(8),
                                      child: isPdf
                                  ...
                                  subtitle: Padding(
                                    ...
                                    child: Row(
                                      children: [
                                        ...
                                        if (fileSizeText.isNotEmpty && formattedDate.isNotEmpty)
                                          Container(
                                  ...
                                  trailing: PopupMenuButton<String>(
                                      ...
                                      if (action == 'delete') {
                                        final confirm = await showDialog<bool>(
                                          ...
                                          builder: (context) => AlertDialog(
                                            ...
                                            content: Text("Are you sure you want to delete '$fileName' permanently from storage?"),
                                            actions: [
                                              TextButton(
                                    ...
                                    itemBuilder: (context) => [
                                      const PopupMenuItem(
                                        ...
                                        child: Row(
                                          children: [
                                            Icon(Icons.share, color: Color(0xFF00A86B), size: 20),
                                            SizedBox(width: 8),
                                            Text('Share'),
                                          ],
                                        ),
                                    ...
                                    child: const Icon(Icons.more_vert, size: 22, color: Colors.grey),
                                  ),
                                  onTap: () {
                                    if (isPdf) {
                                      _openPdfFile(file);
                            ...
                            children: [
                              Expanded(
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text("ID Card Mode", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                ),
                              ),
                              ...
                              Expanded(
                                child: ElevatedButton.icon(
                                  ...
                                  label: const Text("Scan Document", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
                                ),
                              ),
                            ],
                          ),
